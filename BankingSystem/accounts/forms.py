...
class UserRegistrationForm(UserCreationForm):
    accaccount_type = (forms.ModelChoiceField(queryset=BankAccountType.objects.all()))
    gender = forms.ChoiceField(choices=GENDER_CHOICE)
    birth_date = forms.DateField(widget=forms.DateInput(attrs={'type': 'date'}))
    first_name = forms.CharField(widget=forms.TextInput(attrs={'autofocus':'on'}))
    last_name = forms.CharField(widget=forms.TextInput(attrs={'autofocus':'on'}))
    ...
    def save(self, commit = True):
        ...
        if commit:
            ...
            gender = self.cleaned_data.get('genser')
